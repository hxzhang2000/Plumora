...
- ① 新增 GitHub Pages 自动发布：`.github/workflows/deploy-pages.yml`（push main / workflow_dispatch 触发，`npm ci` + `npm run build` → `actions/configure-pages` + `upload-pages-artifact` + `deploy-pages`，产物 `apps/web/dist` 纯静态；hash 路由 + 相对 base 兼容子路径部署，无需服务端 rewrite；仓库 Settings → Pages 的 Source 选「GitHub Actions」即启用）
...
- ① 450 条卦辞爻辞原文入库：64 卦辞 + 384 爻辞 + 2 用九用六，来源维基文库《周易正義》（zh-hant 权威层 → zh-hans 交付层），`hexagrams.ts` 的 `guaci` / `lines` / `yongText` 由 null 填为实际文本
- ② 卦级原文聚合展示面板（`GuaciYaoCiPanel.vue`，806 行）：主区直开（卦辞 / 爻辞 / 用九用六），次区手风琴（彖 / 大象 / 小象 / 文言默认折叠），动爻三重视觉信号（朱砂圆点 + 底色 + 左朱砂边框），一键复制；结果页 `hasGuaciData` 自动切换聚合面板 / 回退卡
- ③ 三传数据入库：彖 64 + 大象 64 + 文言 2（乾/坤）+ 小象 252（62 卦/252 条爻辞），`hexagrams.ts` 新增 `RAW_XI` 常量，手风琴区域渲染彖/大象/小象/文言
- ④ 十翼数据解析入库：系辭上傳 + 系辭下傳 + 說卦 + 序卦 + 雜卦 5 章，`shiYi.generated.ts`（65.8 KB）+ `shiYi.ts`（9 接口 + 5 函数），21 条测试全过
- ⑤ 十翼原文展示 UI（`ShiYiPanel.vue`，700+ 行）：概览页（5 章卡片）+ 阅读页（单页滚动 / 章节侧栏 / 字号控制 7 档 / 键盘导航 ←→/Esc），集成到「学习」页第三个 tab；数据空时优雅降级显示「数据校对中」
- ⑥ 六十四卦页面接入实际原文数据：`LearnView.vue` 移除 M1 占位文案，改为内嵌 `GuaciYaoCiPanel` 组件；`ResultPanel.vue` 回退文案精简
- ⑦ 桌面布局修复：`GuaciYaoCiPanel` desk 模式 `max-width` 由 780px 放宽为 100%（上限由父容器 `--content-max: 1080px` 控制），电脑模式占满内容区
- ⑧ 移除手机框预览：删除 `DeviceFrameToggle` 按钮及相关状态导入，顶栏仅保留布局切换 / 扫码 / 设置
- ⑨ 契约文档同步：09 文档升 v2.3（三传入库状态更新 / 十翼存储结构引用）；`docs/dev/README.md` 升 v1.21；三份文档自查脚本全绿
- ⑩ 十翼 UI + 存储结构设计文档 v0.2（81 KB / 1395 行）：UI 设计规格 + 存储结构设计（9 接口、RAW 元组模式、L1→L3 五阶段解析管线）

## [0.3.0] - 2026-09-26

### 变更

- ① 时间起卦支持手动输入时间：时间来源分段控件（当前时刻 / 公历 / 农历），农历页签含闰月勾选与公历回显、年份 1900–2100、时 0–23，统一走 `castByTime` 路径；删除旧「手动修正」三格，年支随输入年份推导（不再钉死为今天）；`@plumora/lunar` 新增 `lunar2solar` / `leapMonth` / `monthDays` / `leapDays` / `isValidSolarDate` 反查助手；输入标签删除「（上卦）/（下卦）」后缀
- ② 起卦与结果页清除公式推导文案：起卦上下文只保留事实回显（删除 S1 / S2 与 ÷8 余…算式行），结果页时间上下文改事实标签（「丙午年 八月十五 午时 · 动爻 1」式），删除「手写笔记口径 / 体用口径」与「体用以本卦为准」等说明文字
- ③ 结果页重设计为三块切换器（本卦 / 互卦 / 变卦，tablist + 单一 tabpanel，roving tabindex + 方向键 / Home / End / aria-selected）：面板内卡片组随 tab 整体替换，结论卡仅在本卦视图显示；互 / 变视图为该卦自身事实卡（互卦构成 / 动爻与变爻 / 上下卦类象 / 错卦·综卦）；`HexagramStage` 体用改为独立栅格列胶囊标签（仅本卦，互 / 变不标），上 / 下卦之间新增空带（lg 26px / md 22 / sm 16）
- ④ 全域笔画表：由「一级 3500 + 常用 4500 录入表」改为生成式三层结构（`npm run knowledge:build`），Unihan 18.0 `kTotalStrokes` 底座覆盖 URO + 扩展 A 全域 27,584 码点（与 `isHanChar` 同口径，生成物 `strokes.generated.ts` 入库、JSON 仅按需导出），叠加现表 117 冻结字 + hanzi-writer-data 对拍 461 字裁决覆盖层（`npm run knowledge:adjudicate`）；繁体别名表由 30 条扩至 2,291 条（`kSimplifiedVariant` 生成 + 冻结 30），歧义字（后 / 发 等）不换面、按字面取值、繁体成员自建键；新增 `STROKES_SOURCE` 溯源常量；`華` 繁体笔画取 12 并标注「待辞书核对」（Unihan 12 / hwd 11 / shunshi-kangxi 10 三方不一致）
- ⑤ 桌面形态隐藏底部 Tab（底栏仅手机形态 / 框内显示）；二维码常驻入口三处（顶栏圆钮 / 侧栏整行 / 设置面板原入口）共用一个弹窗（新增共享 store `qrDialog`）；宽屏 + 手机形态自动套真机框预览（390 × 844 机身），「退出预览」底部按钮删除、仅留顶栏框图标 / 布局切换出口；扫码地址按地址段挑真实局域网（RFC1918 私网优先、跳过 Tailscale 等 CGNAT 100.64/10 虚拟网卡与 VMware 等虚拟网卡宿主地址 x.x.x.1、link-local），不再盲取 `resolvedUrls.network[0]`
- ⑥ 顶栏两行化：第一行品牌独占、第二行操作钮右对齐 + 淡墨分隔线；≤ 380px 撤销按钮收为字标
- ⑦ 契约文档同步升版：01/03/06/07 → v3.0（手动输入时间口径、结果页切换器与体用标注、顶栏两行 / 桌面隐藏底栏 / 二维码三入口、TC-ST04 改域外字 `𠀀`），05 → v2.0（生成式笔画表三层结构、收字域 27,584、国标《通用规范汉字表》口径修正为 8,100），docs/dev/README 镜像 v1.17；三条文档自查脚本全绿（docset-selfcheck 21 项 / doc-case-check 56 pass / kb-consistency 64 卦三方一致）

## [0.2.0] - 2026-09-26

### 变更

- 移除声音（点数）起卦：起卦方式由五种减为四种（时间 / 数字 / 汉字笔画 / 随机），起卦页、学习页、设置项同步删除该入口
- 移除一数起卦与一字起卦：数字起卦仅保留两数输入、汉字起卦仅保留两字输入，起卦页删除模式切换；单数/单字输入改为报错拒绝（`CastInputError`）
- 学习页删除「起卦方法说明」栏目，保留八卦类象速查与六十四卦速查（进入学习页默认展示八卦类象）
- 删除 `docs/ui/` 单文件 UI 原型及其检查链路：自查脚本移除「原型 HTML 漂移」与「原型 ↔ 交付交叉一致性」两组断言，文档删除原型命令与目录引用
- 旧数据兼容：已保存的「声音起卦」「一数/一字」卦例只读保留、不做迁移回写，列表与详情的方式标签回退为「其他方式」，详情页入参上下文仍按原样渲染；设置里残留的「声音」默认方式载入时回落为「时间」
- 契约文档同步升版：01/03/06/07 删改章节升 v2.0（一数/一字与起卦方法说明改为「已移除」注记），02/04/06/07/00 与索引 README 随引用清理升次版本
